resource "proxmox_virtual_environment_file" "user_cloud_config" {
  ...
  source_raw {
    ...
    write_files:
      ...
      - path: /etc/systemd/system/ansible-pull.service
        ...
        content: |
          ...
          [Service]
          Type=oneshot
          ExecStart=/usr/bin/ansible-pull -U ${var.repo_url} ${var.playbook} --tags="update" -i localhost, -C main
    ...
    runcmd:
      ...
      - ansible-pull -U ${var.repo_url} ${var.playbook} --tags="install" > /var/log/ansible-pull.log 2>&1 || touch /tmp/ansible-pull-failed
